...
| 版 | 場所 | 内容 |
|---|---|---|
| **v8 (現行)** | `index.html` / git tag `v8` | 「FILM JOURNEY」— スクロール連動シーク映像章 (受賞作6章リール25s・scrollytelling)。v7の全機能保持 |
| **v7 (保存)** | `v7.html` / git tag `v7` | 「深度と遊び心」— 4ペルソナ監査を反映した v6 強化版。本物の遠近カメラ (hero perspective+translateZ)・カーソル連動光源/グレア・見出しの奥行き登場・フィルムスプロケット付きマーキー・フクをつつくと「にゃ」・スクロール映写リール・紙グレイン+金箔スイープ・focus-visible・FAQ 2列。lightbox不可視バグ修正済 |
...
保存版はそのまま閲覧可能:
- v7: https://ryominami.github.io/Movie-portfolio/v7.html
